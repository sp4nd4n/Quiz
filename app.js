...
// Shortens a link via is.gd's free, key-free API. Falls back to the
// original link if the request fails for any reason (offline, service down, etc).
...
$('btn-done-building').addEventListener('click', async () => {
  ...
  try {
    ...
    const fullLink = buildQuizLink(code);
    btn.textContent = 'Shortening link…';
    generatedLink = await shortenLink(fullLink);
    $('link-text').textContent = generatedLink;
    await db.collection('quizzes').doc(code).update({ shortLink: generatedLink });
...
function renderQuizzes() {
  ...
  list.innerHTML = quizzesCache.map((q, i) => `
    <div class="resp-card" data-idx="${i}">
      <div class="resp-top">
        <span class="resp-name">${escapeHtml(q.genreName || q.genre || 'Quiz')}</span>
      ...
      <div class="link-box" style="margin-top:12px">
        <span class="qz-link">${escapeHtml(q.shortLink || buildQuizLink(q.id))}</span>
  ...
  list.querySelectorAll('[data-copy-idx]').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      await copyTextToClipboard(q.shortLink || buildQuizLink(q.id));
...
// ---------- manual link shortener ----------

$('btn-open-shorten').addEventListener('click', () => {
  $('shorten-input').value = '';
  $('shorten-error').style.display = 'none';
  $('shorten-result-box').style.display = 'none';
  showScreen('screen-shorten');
});

$('btn-do-shorten').addEventListener('click', async () => {
  const raw = $('shorten-input').value.trim();
  const errEl = $('shorten-error');
  const boxEl = $('shorten-result-box');
  errEl.style.display = 'none';
  boxEl.style.display = 'none';
  if (!raw) { errEl.textContent = 'Paste a link first'; errEl.style.display = 'block'; return; }

  const btn = $('btn-do-shorten');
  btn.disabled = true;
  ...
  try {
    ...
    if (data && data.shorturl) {
      $('shorten-result-text').textContent = data.shorturl;
      $('btn-copy-shortened').dataset.link = data.shorturl;
      $('btn-copy-shortened').textContent = 'Copy';
      $('btn-copy-shortened').classList.remove('copied');
      boxEl.style.display = 'flex';
    } else {
      throw new Error((data && data.errormessage) || 'Unknown error');
    }
  } catch (err) {
    console.error(err);
    errEl.textContent = "Couldn't shorten that right now — try again in a moment.";
...
$('btn-copy-shortened').addEventListener('click', async () => {
  const link = $('btn-copy-shortened').dataset.link;
  if (!link) return;
  await copyTextToClipboard(link);
  $('btn-copy-shortened').textContent = 'Copied ✓';
  $('btn-copy-shortened').classList.add('copied');
  showToast('Link copied to clipboard');
});
